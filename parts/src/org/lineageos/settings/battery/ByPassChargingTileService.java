...
public class ByPassChargingTileService extends TileService {
    ...
    public void onStartListening() {
        ...
        updateUI(sharedPrefs.getBoolean(BYPASSCHARGING_ENABLE_KEY, false));
    }
    ...
    public void onClick() {
        super.onClick();
        SharedPreferences sharedPrefs = PreferenceManager.getDefaultSharedPreferences(this);
        final boolean enabled = !(sharedPrefs.getBoolean(BYPASSCHARGING_ENABLE_KEY, false));
        FileUtils.writeLine(BYPASSCHARGING_NODE, enabled ? "1" : "0");
        sharedPrefs.edit().putBoolean(BYPASSCHARGING_ENABLE_KEY, enabled).commit();
        updateUI(enabled);
    }
}
